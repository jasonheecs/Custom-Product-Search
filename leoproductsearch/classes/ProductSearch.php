...
class ProductSearch
{
	...
	public static function find($id_lang, $conds, $page_number = 1, $page_size = 1, $order_by = 'position',
		$order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
	{
		...
		'.$sql_groups .
		($conds["cate"]? ' AND c.`id_category` = '. $conds["cate"] : ''));
		...
		if ($ajax)
		{
			...
			foreach ($tempResults as $row){
				$row['image_link'] = Context::getContext() ->link->getImageLink($row['prewrite'], $row['id_image'], 'small_default');
				$results[] = $row;
			}
			return $results;
